CREATE PROCEDURE "SRC"."PROC_LI_SRC_ACC_H_T_COMMISSION_FEE"(IN P_INPUT_JOB_SESSION VARCHAR(255),IN P_INPUT_DATADATE VARCHAR(8),OUT P_OUT_WF_STATUS VARCHAR(1))
LANGUAGE SQLSCRIPT
SQL SECURITY INVOKER
DEFAULT SCHEMA SRC -- 存储过程中计算逻辑中的指标表可以不加SCHEMA
AS
BEGIN
/********************************************************************************
...
WHERE TO_CHAR(END_DATE, 'YYYYMMDD') >= :P_INPUT_DATADATE AND TO_DATS(END_DATE) < '99991231';
...
FROM "SRC"."LI_SRC_ACC_H_T_COMMISSION_FEE" SOURCE_T, :TEMP_SRC_DATA AS INCRE_T
